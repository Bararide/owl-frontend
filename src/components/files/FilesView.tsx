...
const findReferencedFilesInText = (content: string, files: ApiFile[]) => {
  const result: Array<{ label: string; file: ApiFile; start: number; end: number; }> = [];
  ...
  files.forEach((file) => {
    ...
    candidates.forEach((candidate) => {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        if (!used.has(key)) {
          used.add(key);
          result.push({ label: full, file, start, end });
        }
      }
    });
  });
...
const LinkedFileTextPanel: React.FC<{
  ...
  const linkedContent = useMemo(() => {
    ...
    refs.forEach((ref, index) => {
      ...
      nodes.push(
        <Box key={`l-${index}`} component="span" sx={{
          color: '#90caf9',
  ...
  return (
    <Paper elevation={0} sx={{
      ...
      <Box sx={{ px: 2, py: 1.5, borderBottom: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)' }}>
        <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>{title}</Typography>
        <Typography variant="caption" color="text.secondary" noWrap>{file?.path || 'No linked file selected'}</Typography>
      </Box>
      <Box sx={{ flex: 1, minHeight: 0, overflow: 'auto', p: 2 }}>
        {!file ? (
          <Typography variant="body2" color="text.secondary">Click a file reference in the main text to open it here.</Typography>
        ) : isLoading ? (
          <Stack alignItems="center" justifyContent="center" sx={{ height: '100%' }} spacing={2}>
            <CircularProgress size={24} />
            <Typography variant="body2" color="text.secondary">Loading linked file...</Typography>
          </Stack>
        ) : error ? (
          <Typography variant="body2" color="error.main">{error instanceof Error ? error.message : 'Failed to load linked file'}</Typography>
        ) : (
          <Box sx={{ fontFamily: '"Fira Code", monospace', fontSize: '0.82rem', lineHeight: 1.65, whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: 'rgba(255,255,255,0.92)' }}>
            {linkedContent}
          </Box>
        )}
      </Box>
...
const FileContentDialog: React.FC<FileContentDialogProps> = ({
  ...
  useEffect(() => {
    if (file?.name && open) {
      ...
      setMatchCase(false); setWholeWord(false); setLinkedFile(null);
  ...
  useEffect(() => {
    if (!open) {
      ...
      setIsSearchActive(false); setCopied(false); setMatchCase(false); setWholeWord(false); setLinkedFile(null);
    }
  }, [open]);
  useEffect(() => {
  ...
  const renderTextWithLinksAndHighlights = () => {
    ...
    const references = findReferencedFilesInText(editedContent, allFiles).filter((ref) => ref.file.path !== file?.path);
    const breakpoints = new Set<number>([0, editedContent.length]);
    flattenRanges.forEach((r) => { breakpoints.add(r.start); breakpoints.add(r.end); });
    references.forEach((r) => { breakpoints.add(r.start); breakpoints.add(r.end); });
    ...
    for (let i = 0; i < sortedPoints.length - 1; i++) {
      ...
      if (reference) {
        node = (<Box component="span" onClick={() => setLinkedFile(reference.file)} sx={{
          color: '#90caf9', textDecoration: 'underline', textDecorationColor: 'rgba(144,202,249,0.55)',
          cursor: 'pointer', '&:hover': { color: '#bbdefb', backgroundColor: 'rgba(144,202,249,0.08)' },
        }}>{text}</Box>);
      }
      if (highlight) {
        node = (<mark data-current-match={highlight.isCurrent ? 'true' : 'false'} style={{
          backgroundColor: highlight.color, color: 'inherit', padding: '0 2px', borderRadius: '3px',
          fontWeight: highlight.isCurrent ? 'bold' : 'normal',
          border: highlight.isCurrent ? '2px solid #ff9800' : 'none',
          boxShadow: highlight.isCurrent ? '0 0 8px rgba(255, 152, 0, 0.5)' : 'none', transition: 'all 0.2s ease',
        }}>{node}</mark>);
      }
        ...
        <DialogContent sx={{ p: 0, flex: 1, display: 'flex', minHeight: 0, overflow: 'hidden' }}>
          ...
          <Box sx={{ flex: 1, minWidth: 0, display: 'flex', overflow: 'hidden' }}>
                  ...
                  <Paper elevation={0} sx={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 2, overflow: 'hidden' }}>
                    <Box sx={{ px: 2, py: 1.25, borderBottom: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)', flexShrink: 0 }}>
                      <Stack direction="row" justifyContent="space-between" alignItems="center">
                        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                          <Chip label={getLanguageFromMimeType(file?.mime_type || '')} size="small" color="primary" />
                          {linkedFile && <Chip icon={<OpenInNewIcon />} label={`Linked: ${linkedFile.name}`} size="small" variant="outlined" />}
                        </Stack>
            ...
            <Box sx={{ display: { xs: 'none', lg: 'block' }, width: 380, minWidth: 380, maxWidth: 420, p: 2, pl: 0, overflow: 'hidden' }}>
              <LinkedFileTextPanel file={linkedFile} containerId={containerId} allFiles={allFiles} title="Linked file" />
            </Box>
          </Box>
        </DialogContent>
        <DialogActions sx={{ borderTop: '1px solid rgba(255,255,255,0.1)', p: 2, background: 'rgba(0,0,0,0.1)', justifyContent: 'space-between' }}>
          <Typography variant="caption" color="text.secondary">Links to referenced files open in the right panel</Typography>
...
export default function FilesView({ containerId }: FilesViewProps) {
  // ✅ Исправлено: добавлено `data:` перед переменными
  ...
  const handleClearSearch = useCallback(() => { setSearchQuery(''); setIsSemanticSearch(false); setSearchResults([]); }, []);
  ...
  return (
    <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#000' }}>
      ...
      <FileContentDialog open={fileContentDialog.open} onClose={handleCloseFileContent} file={fileContentDialog.file} containerId={containerId} allFiles={files} onFileUpdated={refetchFiles} onFileDeleted={refetchFiles} searchQuery={searchQuery} currentFileIndex={fileContentDialog.currentIndex} totalFiles={currentFilesList.length} onNextFile={handleNextFile} onPrevFile={handlePrevFile} />
