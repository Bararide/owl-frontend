...
const ExplanationPanel: React.FC<{ explanation: string; query?: string }> = ({
  explanation,
  query,
}) => (
  <Paper
...
const FileContentDialog: React.FC<
...
const SemanticGraphCanvas = React.memo(
  ({
    files,
    graphData,
    semanticResults,
    ...
    // 🔹 1. ДАННЫЕ ДЛЯ ФИЗИКИ (НЕ ЗАВИСЯТ ОТ ГРУПП)
    const graph = useMemo(
      () => normalizeGraph(files, graphData),
      [files, graphData],
    );

    // 🔹 2. ДАННЫЕ ДЛЯ ОТРИСОВКИ (ЗАВИСЯТ ОТ ГРУПП)
    ...
    // 🔹 ИНИЦИАЛИЗАЦИЯ ТОЛЬКО ПРИ ИЗМЕНЕНИИ ФАЙЛОВ/ГРАФА, А НЕ ГРУПП
    useEffect(() => {
        ...
        <canvas
          ...
          onWheel={(e) => {
            ...
            zoomRef.current = Math.max(
              0.1,
              Math.min(2, zoomRef.current * delta),
            );
          }}
          onMouseDown={(e) => {
          ...
          onMouseMove={(e) => {
            ...
            const node =
              idx >= 0 && idx < nodeIds.length
                ? graph.nodes.find((n) => n.id === nodeIds[idx])
                : null;
